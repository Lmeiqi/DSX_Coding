...
adsl <- adsl %>%
  derive_vars_extreme_event(
    ...
    events = list(
      ...
      event(
        ...
        condition = (!is.na(AESTDTC) & AESDTH!="Y"),
        set_values_to = exprs(
      ...
      event(
        ...
        condition = (!is.na(DSSTDTC) & DSDECOD!="DEATH"),
        set_values_to = exprs(
    ...
    source_datasets = list(ae = ae, adsl = adsl, vs=vs, dm=dm, ex=ex, ds=ds),
    tmp_event_nr_var = event_nr,
  ...
  derive_var_merged_exist_flag(
    dataset_add = vs,
    by_vars = exprs(STUDYID, USUBJID),
    filter_add = (VSTESTCD == "SYSBP" & VSSTRESU == "mmHg"),
    new_var = ABNSBPFL,
    condition = (VSSTRESN >= 140 | VSSTRESN < 110),
    false_value = "N",
    missing_value = "N"
  ) %>%
  derive_var_merged_exist_flag(
